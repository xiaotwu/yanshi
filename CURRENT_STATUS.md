...
- **Workshop character redesign Increments 5/6**: the owner selected and visually signed off Concept A
  (Paper-Lantern Dragon Apprentice). The shared `MascotRig` now has the honest `deriveMascotState`
  selector, six tokenized role skins (manager, browser, computer, file, reviewer, terminal), and a
  Workshop view-model wired from real `runs`, `events`, `approvals`, `providerHealth`, active run id,
  reduced-motion, and document visibility. `WorkerRail`, `WorkerInspector`, and `AtelierPreview`
  render the shared rig; Workshop hides the old 3D worker standees so there is one visible mascot
  source.
...
- Desktop: `pnpm --filter @yanshi/desktop test` -> 25 files / 117 tests passed; `typecheck` -> pass; `build` -> pass (existing Vite dynamic-import/chunk-size warnings).
